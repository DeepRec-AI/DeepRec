...
class Model(object):
    def __init__(self, n_uid, n_mid, n_cat, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling = False):
        ...
        with tf.name_scope('Embedding_layer'):
            ...
            self.uid_batch_embedded = tf.nn.embedding_lookup(self.uid_embeddings_var, math_ops.cast(self.uid_batch_ph,dtypes.int64))

            self.mid_embeddings_var = tf.get_variable("mid_embedding_var", [n_mid, EMBEDDING_DIM])
            self.mid_batch_embedded = tf.nn.embedding_lookup(self.mid_embeddings_var, math_ops.cast(self.mid_batch_ph, dtypes.int64))
            self.mid_his_batch_embedded = tf.nn.embedding_lookup(self.mid_embeddings_var, math_ops.cast(self.mid_his_batch_ph,dtypes.int64))
            if self.use_negsampling:
                self.noclk_mid_his_batch_embedded = tf.nn.embedding_lookup(self.mid_embeddings_var, math_ops.cast(self.noclk_mid_batch_ph, dtypes.int64))

            self.cat_embeddings_var = tf.get_variable("cat_embedding_var", [n_cat, EMBEDDING_DIM])
            self.cat_batch_embedded = tf.nn.embedding_lookup(self.cat_embeddings_var, math_ops.cast(self.cat_batch_ph, dtypes.int64))
            self.cat_his_batch_embedded = tf.nn.embedding_lookup(self.cat_embeddings_var, math_ops.cast(self.cat_his_batch_ph, dtypes.int64))
            if self.use_negsampling:
                self.noclk_cat_his_batch_embedded = tf.nn.embedding_lookup(self.cat_embeddings_var, math_ops.cast(self.noclk_cat_batch_ph, dtypes.int64))
