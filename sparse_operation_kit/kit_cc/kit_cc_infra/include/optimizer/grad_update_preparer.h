...
public:
    static std::shared_ptr<GradUpdatePreparer> create(const size_t global_batch_size,
...
private:
    GradUpdatePreparer(const size_t global_batch_size, const size_t max_feature_num,
                       const size_t max_vocabulary_size_per_gpu,
            std::vector<std::shared_ptr<HugeCTR::GeneralBuffer2<HugeCTR::CudaAllocator>>>& buffers,
            std::vector<std::shared_ptr<HugeCTR::GeneralBuffer2<HugeCTR::CudaHostAllocator>>>& host_buffers,
            std::shared_ptr<ResourcesManager>& resource_mgr);
    ...
    Tensors2<int64_t> sort_duplicated_indices_; // store the sorted duplicated_indices TODO: make it template
