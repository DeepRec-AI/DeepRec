...
GradUpdatePreparer::GradUpdatePreparer(const size_t global_batch_size, 
                                       const size_t max_feature_num,
...
: global_batch_size_(global_batch_size), max_feature_num_(max_feature_num),
max_vocabulary_size_per_gpu_(max_vocabulary_size_per_gpu),
buffers_(buffers), host_buffers_(host_buffers), resource_mgr_(resource_mgr)
{
    ...
    for (size_t dev_id = 0; dev_id < local_gpu_count; dev_id++) {
        ...
        {
            size_t size = 0;
            InclusiveSum((void*)nullptr, size, (uint32_t *)nullptr,
                         (uint32_t *)nullptr, 
        ...
        {
            size_t size = 0;
            SortPairs((void*)nullptr, size, (int64_t *)nullptr, (int64_t *)nullptr,
...
std::shared_ptr<GradUpdatePreparer> GradUpdatePreparer::create(const size_t global_batch_size,
                                                               const size_t max_feature_num,
                                                               const size_t max_vocabulary_size_per_gpu,
            std::vector<std::shared_ptr<HugeCTR::GeneralBuffer2<HugeCTR::CudaAllocator>>>& buffers,
            std::vector<std::shared_ptr<HugeCTR::GeneralBuffer2<HugeCTR::CudaHostAllocator>>>& host_buffers,
            std::shared_ptr<ResourcesManager>& resource_mgr) {
    return std::shared_ptr<GradUpdatePreparer>(new GradUpdatePreparer(global_batch_size, max_feature_num,
...
void GradUpdatePreparer::operator()(const std::shared_ptr<Tensor>& duplicated_indices, 
    ...
    gen_position_for_indices(duplicated_indices->GetPtrWithType<int64_t>(),
                             nnz,
                             positions_[local_replica_id].get_ptr(),
                             local_gpu->get_stream());

    // step2: sort duplicated_indices
    int32_t end_bit = static_cast<int32_t>(std::log2(static_cast<float>(max_vocabulary_size_per_gpu_))) + 1;
    ...
    gen_unique_flags_for_indices(sort_duplicated_indices_[local_replica_id].get_ptr(),
                                 nnz, indices_unique_flags_[local_replica_id].get_ptr(),
                                 local_gpu->get_stream());
    ...
    gen_unique_indexes_for_indices(indices_unique_flags_[local_replica_id].get_ptr(),
                                   indices_unique_sums_[local_replica_id].get_ptr(),
                                   nnz, 
...
std::shared_ptr<Tensor>& GradUpdatePreparer::get_indices_unique_indexes(const size_t local_replica_id) {
