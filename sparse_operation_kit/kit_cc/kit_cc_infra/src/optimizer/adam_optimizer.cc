...
std::shared_ptr<AdamOptimizer> AdamOptimizer::create(optimizer_hyper_params&& hyper_params,
                                                     const std::shared_ptr<ParamsManager>& params_mgr,
                                                     std::shared_ptr<ResourcesManager> resource_mgr) {
    return std::shared_ptr<AdamOptimizer>(new AdamOptimizer(std::move(hyper_params), params_mgr, resource_mgr));
}

AdamOptimizer::AdamOptimizer(optimizer_hyper_params&& hyper_params,
                             const std::shared_ptr<ParamsManager>& params_mgr,
                             std::shared_ptr<ResourcesManager> resource_mgr) 
: Optimizer(params_mgr), resource_mgr_(resource_mgr),
...
std::shared_ptr<UpdatePreparer>& AdamOptimizer::get_update_preparer(const std::string variable_name) {
...
void AdamOptimizer::apply_gradients(std::shared_ptr<ParamInterface> param,
    ...
    opt_adam_update_states_global(indices_unique_indexes->GetPtrWithType<size_t>(),
                                  sorted_position->GetPtrWithType<int64_t>(),
                                  sorted_indices->GetPtrWithType<int64_t>(),
                                  unique_indices_num,
                                  grad_tensor->GetPtrWithType<float>(),
                                  param->get_embedding_vec_size(),
                                  beta1_, beta2_,
                                  m_states->GetPtrWithType<float>(),
                                  v_states->GetPtrWithType<float>(),
                                  stream);

    // update embedding variable
    opt_adam_update_param_global(param->get_embedding_vec_size(),
                                 param->get_max_vocabulary_size_per_gpu(),
                                 alpha_t, beta1_, beta2_, epsilon_, 
                                 m_states->GetPtrWithType<float>(),
                                 v_states->GetPtrWithType<float>(),
                                 emb_table_tensor->GetPtrWithType<float>(),
                                 stream);
}
