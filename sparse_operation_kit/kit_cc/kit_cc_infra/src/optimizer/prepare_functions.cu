...
void gen_position_for_indices(const int64_t* indices, const size_t elem_num,
    ...
    gen_position_for_indices_kernel<<<grid_size, block_size, 0, stream>>>(
        indices, elem_num, positions);
}

__global__ void gen_unique_flags_for_indices_kernel(const int64_t* indices, 
                                            const size_t elem_num, uint32_t* flags) {
    size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    for (size_t i = gid; i < elem_num; i += stride) {
        ...
        if (i > 0) {
            ...
            if (cur_indice != former_indice) flags[i] = 1; // it is an unique indice
            else flags[i] = 0; // it is not an unique indice
        } else {
...
void gen_unique_flags_for_indices(const int64_t* indices, const size_t elem_num,
    ...
    gen_unique_flags_for_indices_kernel<<<grid_size, block_size, 0, stream>>>(
                                                    indices, elem_num, flags);
}

__global__ void gen_unique_indexes_for_indices_kernel(const uint32_t* flags, const uint32_t* prefix_sums,
