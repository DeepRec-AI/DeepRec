...
cudaError_t SortPairs(void *d_temp_storage, size_t &temp_storage_bytes,
                      ...
                      size_t num_items, int32_t begin_bit, int32_t end_bit,
                      cudaStream_t stream, bool debug_synchronous) {
    return cub::DeviceRadixSort::SortPairs(d_temp_storage, temp_storage_bytes, d_keys_in, 
...
template cudaError_t SortPairs(void *d_temp_storage, size_t &temp_storage_bytes,
                      ...
                      const int64_t *d_values_in, int64_t *d_values_out,
                      size_t num_items, int32_t begin_bit, int32_t end_bit,
                      cudaStream_t stream, bool debug_synchronous);
template cudaError_t SortPairs(void *d_temp_storage, size_t &temp_storage_bytes,
...
template cudaError_t InclusiveSum(void *d_temp_storage, size_t &temp_storage_bytes,
...
__device__ __forceinline__ float accumulate_gradients(const size_t* unique_indexes, 
                                                      ...
                                                      const size_t bid, const size_t tid) {
...
__global__ void opt_adam_update_states_global_kernel(const size_t* unique_indexes, 
                                const int64_t* sorted_position, const int64_t* sorted_indices,
                                const uint32_t unique_indices_num, 
    ...
    if (tid < emb_vec_size && bid < unique_indices_num) {
        float summed = accumulate_gradients(unique_indexes, sorted_position, 
                                            emb_vec_size, gradients, bid, tid);
...
void opt_adam_update_states_global(const size_t* unique_indexes, const int64_t* sorted_position, 
    ...
    opt_adam_update_states_global_kernel<<<grid_size, block_size, 0, stream>>>(
        unique_indexes, sorted_position, sorted_indices, unique_indices_num, gradients,
        emb_vec_size, beta1, beta2, m, v);
}


__global__ void opt_adam_update_param_global_kernel(
                                  const size_t emb_vec_size, const size_t max_vocabulary_size,
                                  const float alpha_t, const float beta1, const float beta2,
                                  const float epsilon, float* m, float* v, 
...
void opt_adam_update_param_global(const size_t emb_vec_size, const size_t max_vocabulary_size,
                                  const float alpha_t, const float beta1, const float beta2,
                                  const float epsilon, float* m, float* v, 
                                  float* embedding_table, cudaStream_t stream) {
