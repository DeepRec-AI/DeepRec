...
Optimizer::Optimizer(const std::shared_ptr<ParamsManager>& params_mgr) 
: params_mgr_(params_mgr)
{}

class OptimizerVariantWrapper {
public:
    OptimizerVariantWrapper() : optimizer_(nullptr) {}
    explicit OptimizerVariantWrapper(const std::shared_ptr<Optimizer> optimizer) : optimizer_(optimizer) {}
