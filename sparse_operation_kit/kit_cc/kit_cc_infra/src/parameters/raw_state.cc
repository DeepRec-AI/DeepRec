...
std::shared_ptr<RawStates> RawStates::create(const std::vector<size_t> shape, 
...
RawStates::RawStates(const std::vector<size_t> shape, 
              const std::shared_ptr<ResourcesManager>& resource_mgr,
              const std::vector<std::shared_ptr<HugeCTR::GeneralBuffer2<HugeCTR::CudaAllocator>>>& buffers,
              const std::string initializer) 
: resource_mgr_(resource_mgr), shape_(shape), 
...
void RawStates::init(const size_t global_replica_id) {
    ...
    if (local_replica_id >= state_tensors_.size())
        throw std::runtime_error(ErrorBase + "local_replica_id is out of the range of state_tensor.size().");

    const auto &local_gpu = resource_mgr_->get_local_gpu(local_replica_id);

    initializer_->fill(state_tensors_interface_[local_replica_id],
...
std::shared_ptr<Tensor>& RawStates::get_tensor(const size_t local_replica_id) {
    if (local_replica_id >= state_tensors_.size())
        throw std::runtime_error(ErrorBase + "local_replica_id is out of the range of state_tensors.size().");
