...
## 背景
DeepRec的EmbeddingVariable之前针对tensorflow原生的embedding lookup API做了单路Embeddig子图Fusion（主要是将EmbeddingLookup中子图的多个Op做fusion）。但在典型的CTR场景，实际上会存在多个Embedding的查询，这种情况我们经过profiling发现仍然会存在框架侧多个Op的kernel launch以及GPU侧多个Cuda Kernel launch的瓶颈，这在Embedding规模较大时会影响训练的性能。我们希望能通过支持多个Embedding的同时聚合查询，从而优化模型中大量Embedding子图查询的性能。

Group Embedding功能支持同时对多个EmbeddingVariable 聚合查询，将原来可能上百个Embedding查询的子图融合成极少节点的子图。GroupEmbedding接口目前支持单卡的Fusion以及分布式的多卡的Fusion，可分为以下几种使用模式：

| API | Device Type | Note |
| ------ | ---------------------- | ---- |
| localized            | CPU/GPU  | 推荐用于单机单卡训练或者将已有的模型重新导出用于推理 |
| collective           | GPU | 推荐用于单机多卡训练 |
| parameter_server(WIP)     | - | 暂未支持 |


下面我们分单机和分布式两种使用方法给大家介绍。

## 单机模式

### 用户接口
单机模式下Group Embedding的接口支持两个层面的API，分别为底层API `tf.nn.group_embedding_lookup_sparse` 、`tf.nn.group_embedding_lookup`
...
- `params` : List, 该参数可以接收一个或者多个EmbeddingVariable或者是原生Tensorflow Variable
- `ids` : List | Tuple , Tensor ，values是用于查找的ID 长度必须和params保持一致
- `partition_strategy` : str 目前暂时不支持
- `name` : str group的名称
...
**group_embedding_lookup_sparse的使用方式如下**
...
with tf.device('/GPU:{}'.format(0)): # 使用CPU GroupEmbedding只需要修改embeddingvar 的device placement为CPU即可
...
with tf.device('/gpu:0'): # 使用CPU GroupEmbedding只需要修改device placement为CPU即可
...
with tf.device('/GPU:{}'.format(0)): #目前只支持GPU
    var_0 = tf.get_embedding_variable("var_0",
                                    embedding_dim=16,
                                    initializer=tf.ones_initializer(tf.float32),
                                    ev_option=ev_opt)
...
with tf.feature_column.group_embedding_column_scope(name="item"): #目前只支持GPU
  ad0_col = tf.feature_column.categorical_column_with_embedding(
      key='ad0', dtype=dtypes.int64, ev_option=ev_opt)
  ad0_fc = tf.feature_column.embedding_column(
    categorical_column=ad0_col,
