...
class TestGpuRadixSortKernel : public tensorflow::OpKernel {
 public:
  ...
  void Compute(tensorflow::OpKernelContext* context) override {
    ...
    // INFO: In the test of the GPU operator, the input tensor is placed in
    // the managed memory, which allows both the host and the GPU to access
    // the input tensor. However, the current MemcpyDevicetoDevice of
    // stream_executor does not support data copying between the managed memory
    // and the device memory, so we have to first copy the input tensor to the
    // device memory.
    se::Stream* stream = context->op_device_context()->stream();
    Tensor d_keys_in;
    OP_REQUIRES_OK(context,
                   context->allocate_temp(DataTypeToEnum<Tkey>::value,
                                          keys_in.shape(), &d_keys_in));
    Tkey* d_keys_in_data = d_keys_in.flat<Tkey>().data();
    se::DeviceMemoryBase d_key_in_gpu_dst(d_keys_in_data, size*sizeof(Tkey));
    OP_REQUIRES(
        context,
        stream->ThenMemcpy(&d_key_in_gpu_dst, keys_in_data, size*sizeof(Tkey))
            .ok(),
        errors::Internal("Failed to copy keys_in to gpu"));

    Tensor d_indices_in;
    OP_REQUIRES_OK(context,
                   context->allocate_temp(DataTypeToEnum<Tindex>::value,
                                          indices_in.shape(), &d_indices_in));
    Tindex* d_indices_in_data = d_indices_in.flat<Tindex>().data();
    int64 indices_size = indices_in.NumElements();
    se::DeviceMemoryBase d_indices_in_gpu_dst(d_indices_in_data,
                                              indices_size*sizeof(Tindex));
    OP_REQUIRES(context,
                stream
                    ->ThenMemcpy(&d_indices_in_gpu_dst, indices_in_data,
                                 indices_size*sizeof(Tindex))
                    .ok(),
                errors::Internal("Failed to copy indices_in to gpu"));
    ...
    OP_REQUIRES_OK(context,
                   GpuRadixSort(context, size, d_keys_in_data, keys_out_data,
                                d_indices_in_data, indices_out_data, num_bits_));
  }
...
class TestGpuSelectFlaggedKernel : public tensorflow::OpKernel {
 public:
  ...
  void Compute(tensorflow::OpKernelContext* context) override {
    ...
    int64 input_size = input.dim_size(0);
    ...
    OP_REQUIRES_OK(context, context->allocate_temp(DT_INT64, TensorShape({}),
                                                   &output_size_t));
    int64* output_size_data = output_size_t.scalar<int64>().data();
