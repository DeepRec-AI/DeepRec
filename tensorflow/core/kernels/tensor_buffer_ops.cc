...
class TensorBufferPutGpuOp : public TensorBufferOp {
 public:
  explicit TensorBufferPutGpuOp(OpKernelConstruction* ctx) : TensorBufferOp(ctx) {
  ...
  void ComputeWithTensorBuf(OpKernelContext* ctx, TensorBuf* buf) override {
    ...
    auto fused_tensor_data = const_cast<char*>(fused_tensor.tensor_data().data());
    ...
    Shard(worker_threads.num_threads, worker_threads.workers, ctx->num_inputs(), cost, copy_task);

    auto* d_context = static_cast<const GPUDeviceContext*>(ctx->op_device_context());
    se::Stream* copy_stream = d_context->host_to_device_stream();
    se::Stream* compute_stream = d_context->stream();
    
    Tensor gpu_tensor;
    OP_REQUIRES_OK(ctx, ctx->allocate_temp(DT_INT8, {total_bytes}, &gpu_tensor));
    
    copy_stream->ThenWaitFor(compute_stream);
    se::DeviceMemoryBase wrapped_dst(const_cast<char*>(gpu_tensor.tensor_data().data()),
                                gpu_tensor.TotalBytes());
    copy_stream->ThenMemcpy(&wrapped_dst, const_cast<char*>(fused_tensor_data),
                            gpu_tensor.TotalBytes()).ok();
    compute_stream->ThenWaitFor(copy_stream);
    
    for(size_t j = 0; j < ctx->num_inputs(); j++) {
      size_t bytes_tensor_offset = input_offsets[j];
      Tensor tensor_slice = gpu_tensor.Slice(bytes_tensor_offset,
          bytes_tensor_offset + ctx->input(j).TotalBytes());
      Tensor output(ctx->input(j).dtype());
      OP_REQUIRES_OK(ctx, output.BitcastFrom(
          tensor_slice, ctx->input(j).dtype(), ctx->input(j).shape()));
...
class TensorBufferPutCpuOp : public TensorBufferOp {
 public:
  explicit TensorBufferPutCpuOp(OpKernelConstruction* ctx) : TensorBufferOp(ctx) {
    OP_REQUIRES_OK(ctx, ctx->GetAttr("timeout_millis", &timeout_millis_));
  }
...
#if GOOGLE_CUDA
REGISTER_KERNEL_BUILDER(Name("TensorBufferPut")
                        .Device(DEVICE_GPU)
                        .HostMemory("record"),
