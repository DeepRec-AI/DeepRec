...
#define DeclareSelectScanKernel                                           \
  ...
  __global__ void SelectScanKernelName(                                   \
      const T* keys, SelectScanKernelArgs, const int64_t num_partitions,  \
      const int64_t length, const int64_t predicates_length,              \
      const int64_t counters_length, unsigned int* predicates,            \
...
#define DeclareSelectKernel                                                    \
  ...
  __global__ void SelectKernelName(                                            \
    ...
    for (int j = 0; j < num_partitions; j++) {                                 \
      ...
      _Pragma("unroll") for (int i = 0; i < 32; i++) {                         \
        unsigned int mask = __shfl(predmask, i);                               \
...
#define DeclareSelect                                                          \
  ...
  void SelectName(OpKernelContext* ctx, const Tensor& keys,                    \
    ...
    cudaMemcpy2DAsync(                                                         \
        ...
        data_p_with_type<void*>(inclusive_sum_counters) + counters_length - 1, \
