...
Status GpuInclusivePrefixSum(OpKernelContext* context, int size,
  ...
  auto err = cub::DeviceScan::InclusiveSum(nullptr, temp_storage_bytes, input,
...
namespace unique_gpu {

template <typename T, typename TIndex>
struct UniqueSubOpGPU {
  // Not really a TensorFlow Op
  void operator()(OpKernelContext* context, const Tensor& input,
                  bool has_count_output, Tensor* unique_keys_out,
    ...
    if (input_size == 0) {
      ...
      AllocateTemp(context, 0, unique_idxs_out, temp);
      if (has_count_output) {
    ...
    OP_REQUIRES_OK(context, GpuInclusivePrefixSum(context, input_size,
