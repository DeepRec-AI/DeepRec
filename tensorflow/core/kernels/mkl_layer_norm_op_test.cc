...
//----------------------------------------------------------------------------//
static Graph* FusedLayerNormalize(const string& kind, int rows, int cols) {
...
#define BM_FusedLayerNorm(kind, ROWS, COLS, NTH)                                    \
  static void BM_FusedLayerNorm##_##kind##_##ROWS##_##COLS##_##NTH##_CPU(           \
  int iters) {                                                                      \
  testing::UseRealTime();                                                           \
  testing::ItemsProcessed(static_cast<int64>(iters) * ROWS * COLS * 3);             \
  SessionOptions opts;                                                              \
  opts.config.set_intra_op_parallelism_threads(NTH);                                \
  test::Benchmark("cpu", FusedLayerNormalize(#kind, ROWS, COLS), &opts).Run(iters); \
  }                                                                                 \
  BENCHMARK(BM_FusedLayerNorm##_##kind##_##ROWS##_##COLS##_##NTH##_CPU);            \

#define BM_FusedLayerNorm_Kind(ROWS, COLS, NTH) \
  BM_FusedLayerNorm(Default, ROWS, COLS, NTH);  \
  BM_FusedLayerNorm(Mkl, ROWS, COLS, NTH);      \

#define BM_FusedLayerNorm_NTH(ROWS, COLS) \
  BM_FusedLayerNorm_Kind(ROWS, COLS, 1);  \
  BM_FusedLayerNorm_Kind(ROWS, COLS, 4);  \
  BM_FusedLayerNorm_Kind(ROWS, COLS, 8);  \
