...
class ParallelHashMap {
 public:
  ParallelHashMap(int min_part_size = 128, int part_count = 32) 
  ...
  void Update(const Tensor& indices, OpKernelContext *ctx) {
    ...
    SplitParallelParts(N, std::min(part_count_, thread_pool.workers->NumThreads()), parts);
...
class IncrKeyDumpIterator : public DumpIterator<K> {
 public:
  IncrKeyDumpIterator(std::vector<K>& incr_keys):incr_keys_(incr_keys)  {
...
class IncrEVValueDumpIterator : public  DumpIterator<T> {
 public:
  IncrEVValueDumpIterator(std::vector<K>& incr_keys, EmbeddingVar<K, T>*& emb_var):incr_keys_(incr_keys), emb_var_(emb_var){
    keys_iter_ = incr_keys_.begin();
    keys_idx_ = 1;
    col_idx_ = 0;
  }
...
class IncrEVFreqDumpIterator : public  DumpIterator<T> {
 public:
  IncrEVFreqDumpIterator(std::vector<K>& incr_keys, EmbeddingVar<K, V>*& emb_var):incr_keys_(incr_keys), emb_var_(emb_var){
    keys_iter_ = incr_keys_.begin();
  }
...
class IndicesIncrRecorder: public ResourceBase {
 public:
  ...
  Status DumpSparseNormalTensor(const string& tensor_name, const Tensor& variable, BundleWriter* writer) {
    mutex_lock l(mu_);
  ...
  Status DumpSparseEmbeddingTensor(const string& tensor_name, EmbeddingVar<K, V>* emb_var, BundleWriter* writer, OpKernelContext* context) {
    ...
    IncrEVVersionDumpIterator<K, V, int64> ev_version_dump_iter(partitioned_incr_keys, emb_var);
    ...
    IncrEVFreqDumpIterator<K, V, int64> ev_freq_dump_iter(partitioned_incr_keys, emb_var);
...
class SparsePartitioner {
 public:
  ...
  std::string toString() const {
    return strings::Printf("part_mode:%s, part_count:%lld, part_idx:%ld, hash_bucket_size:%ld",
