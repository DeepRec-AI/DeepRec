...
class TensorPoolAllocatorGPU : public Allocator {
 public:
  TensorPoolAllocatorGPU(SubAllocator* sub_allocator, string name, 
                      size_t total_memory);
  ~TensorPoolAllocatorGPU() override;

  TensorPoolAllocatorGPU(const TensorPoolAllocatorGPU&) = delete;
  TensorPoolAllocatorGPU& operator=(const TensorPoolAllocatorGPU&) = delete;
  ...
  class VirtualBuffer {
   public:
    VirtualBuffer(std::vector<VirtualAllocBlockGPU*>& vblocks,
 ...
 private:
  ...
  MemoryPlannerBaseGPU* mem_planner_;
