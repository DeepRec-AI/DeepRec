...
//
// A tensor bundle can be accessed randomly using a DBReader.  Usage:
//
//   DBReader reader(env, "/fs/model/train/ckpt-step/ckpt");
//   reader.Lookup("name", &tensor);
//
// A tensor bundle can be built using DBWriter.  Each DBWriter builds a
...
//
//   worker 0:
//     DBWriter writer(env, "/fs/model/train/ckpt-step/tmp/worker0-step");
//     writer.Add(...);  // Adds the tensors on this worker.
//     writer.Finish();  // Flushes.
//   worker 1:
//     DBWriter writer(env, "/fs/model/train/ckpt-step/tmp/worker1-step");
//     writer.Add(...);
//     writer.Finish();
//   worker 2:
//     MergeBundles(env,
...
#ifndef TENSORFLOW_CORE_UTIL_TENSOR_BUNDLE_DB_WRITER_H_
#define TENSORFLOW_CORE_UTIL_TENSOR_BUNDLE_DB_WRITER_H_
...
//
// All threads accessing the same DBWriter must synchronize.
class DBWriter {
 public:
  struct Options {
    Options() {}
    // Alignment, in bytes, for tensor data. //数据对齐
    // Must be >= 1. The default size of 1 densely packs tensors.
    int data_alignment{1};
  };
  DBWriter(Env* env, StringPiece prefix,
               const Options& options = Options());
  DBWriter(Env* env, StringPiece prefix,string db_prefix_tmp_,
               const Options& options = Options());
  // Adds the tensor "val" under key "key".
  // Across calls "key" must be unique but can be added in any order.

  Status Add(StringPiece key, const Tensor& val,std::string tensor_type="normal_tensor");
  ...
  //
  // If a full tensor has slices placed on N devices and N DBWriter's are
  ...
  Status AddSlice(StringPiece full_tensor_key,
                  ...
                  const TensorSlice& slice_spec, const Tensor& slice_tensor,std::string tensor_type="slice_tensor");
 ...
 private:
  ...
  Status status_;
  const string db_prefix_tmp_;
  const string db_prefix_;
  BundleEntryProto* entry_seg_;
  std::vector<std::string> db_key_list_;

  TF_DISALLOW_COPY_AND_ASSIGN(DBWriter);
};
...
// must be checked.
// All threads accessing the same DBReader must synchronize.
class DBReader {
 public:
  DBReader(Env* const env, StringPiece prefix);
  ~DBReader();
 ...
 private:
  ...
  TF_DISALLOW_COPY_AND_ASSIGN(DBReader);
};
