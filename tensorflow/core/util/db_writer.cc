...
string Gen_DBKey_Helper(const string& key, int64 offset) {
  return strings::StrCat(key, ":", offset);
}
...
Status WriteTensor(const Tensor& val, leveldb::DB* dbout_,
...
Status WriteTensor(const Tensor& val, leveldb::DB* dbout_,
                   size_t* bytes_written,const string db_tensor_prefix_) {
  *bytes_written = val.TotalBytes();
  char* buf = GetBackingBuffer(val);
   dbout_->Put(leveldb::WriteOptions(),db_tensor_prefix_,buf);
   return Status::OK();
}
...
Status WriteStringTensor(const Tensor& val, leveldb::DB* dbout_,
                         size_t* bytes_written, uint32* crc32c,const string db_tensor_prefix_) {
  // On-disk format:
  //   [varint64 len0]..[varint64 lenL][4 byte cksum on lengths][string bytes]
  // Var "crc32c" checksums the string lengths (as uint64, not varint64 bytes),
  // the length-checksum, and all the string bytes.
 ...
 // TF_RETURN_IF_ERROR(out->Append(lengths));
  to_written_data.append(lengths); 
  *bytes_written = lengths.size();
...
//,const string db_tensor_prefix_
...
Status WriteVariantTensor(const Tensor& val, leveldb::DB* dbout_,
                          size_t* bytes_written, uint32* crc32c,const string db_tensor_prefix_) {
  // On-disk format:
  //   [varint64 len1][bytes variant1][4 byte checksum]
  //   ..
  ...
  *bytes_written = 0;
  ...
  dbout_->Put(leveldb::WriteOptions(),db_tensor_prefix_,to_written_data);
  return Status::OK();
}
...
DBWriter::DBWriter(Env* env, StringPiece prefix, const Options& options)
    : env_(env),
      options_(options),
      prefix_(prefix),
  ...
  leveldb::Options optionss;
  optionss.create_if_missing = true;
 
  leveldb::Status status = leveldb::DB::Open(optionss,"testdb",&dbout_);
  //dbout_->Put(leveldb::WriteOptions(), "key-build", "value");  test ok
  assert(status.ok());

  
}

DBWriter::DBWriter(Env* env, StringPiece prefix, string db_prefix_tmp,const Options& options)
    : env_(env),
      options_(options),
      prefix_(prefix),
      db_prefix_tmp_(db_prefix_tmp),
      tmp_metadata_path_(strings::StrCat(MetaFilename(prefix_), ".tempstate",
                                         random::New64())),
      tmp_data_path_(strings::StrCat(DataFilename(prefix_, 0, 1), ".tempstate",
                                     random::New64())),
...
Status DBWriter::Add(StringPiece key, const Tensor& val,std::string tensor_type) {
  ...
  auto random_key = random::New64();
  std::string random_key_string = std::to_string(random_key);
  const std::string add_key = std::string(db_prefix_tmp_+"--data--0--1--"+tensor_type+"--"+key_string+"-"+random_key_string);
  db_key_list_.push_back(add_key);
  ...
  entry->set_offset(random_key);  // 修改offset为leveldb的key

  // Updates the data file.  要构建tensor data的key和value，目前已有value
  size_t data_bytes_written = 0;
  uint32 crc32c = 0;
  out_->clear_crc32c();

  if (val.dtype() == DT_STRING) {
    status_ = WriteStringTensor(val, dbout_, &data_bytes_written, &crc32c,add_key);
  } else if (val.dtype() == DT_VARIANT) {
    status_ = WriteVariantTensor(val, dbout_, &data_bytes_written, &crc32c,add_key);
  } else {
    status_ = WriteTensor(val, dbout_, &data_bytes_written,add_key);
    crc32c = out_->crc32c();
  }
...
Status DBWriter::AddSlice(StringPiece full_tensor_key,
                              const TensorShape& full_tensor_shape,
                              const TensorSlice& slice_spec,
                              const Tensor& slice_tensor,std::string tensor_type) {
  if (!status_.ok()) return status_;
  ...
  if (IsFullSlice(slice_spec, full_tensor_shape)) {
    return Add(full_tensor_key, slice_tensor,"slice_tensor");
  }
  ...
  status_ = Add(slice_name, slice_tensor,"slice_tensor");
  return status_;
}

Status DBWriter::AddTensorHeader(StringPiece key, DataType dtype) {
  if (!status_.ok()) return status_;
  CHECK_NE(key, kHeaderEntryKey);
  const string key_string(key);
  if (entries_.find(key_string) != entries_.end()) {
...
//HashTableDBWriter
Status DBWriter::AddSliceHeader(
...
Status DBWriter::AddTensorHeader(StringPiece key, DataType dtype, TensorShape shape) {
  if (!status_.ok()) return status_;
  CHECK_NE(key, kHeaderEntryKey);
  const string key_string(key);
  if (entries_.find(key_string) != entries_.end()) {
...
Status DBWriter::AddCompeleteData(char* content, int64 data_bytes_written) {
   uint32 crc32c = 0;
...
void DBWriter::FillTensorShape(TensorShape shape) {
  shape.AsProto(entry_seg_->mutable_shape());
}
// dump mutiple times;
Status DBWriter::AppendSegmentData(char* content, int64 data_bytes_written) {
  return out_->AppendSegment(StringPiece(content, data_bytes_written));
}

void DBWriter::EndSegmentData(int64 total_bytes_written, int64 end_bytes_written) {
...
Status DBWriter::Finish() {
  ...
  int list_lens = db_key_list_.size();
  std::string key_list_str;
  for (int i = 0; i < list_lens; i++) {
      VLOG(1)<< "DBWriter::Finish()::db_key_list_:" << db_key_list_[i]<<"|qiguai0913";
      std::cout<<"std::cout:"<<db_key_list_[i]<<std::endl;
      key_list_str = strings::StrCat(key_list_str, db_key_list_[i], "|");
  }
  dbout_->Put(leveldb::WriteOptions(),db_prefix_tmp_+"--key_list_str",key_list_str);
  ...
  status_ = errors::Internal("DBWriter is closed");
  return Status::OK();
}
//
...
DBReader::DBReader(Env* env, StringPiece prefix)
    : env_(env),
      prefix_(prefix),
      metadata_(nullptr),
      table_(nullptr),
...
Status DBReader::GetBundleEntryProto(StringPiece key,
...
Status DBReader::GetValue(const BundleEntryProto& entry, Tensor* val) {
  Tensor* ret = val;
...
Status DBReader::Lookup(StringPiece key, Tensor* val) {
  CHECK(val != nullptr);
  BundleEntryProto entry;
...
Status DBReader::LookupHeader(StringPiece tensor_key, int64 total_bytes) {
  BundleEntryProto entry;
...
Status DBReader::LookupSegment(StringPiece key, size_t buffer_size, char* destination, size_t& real_bytes_read) {
...
Status DBReader::LookupSegmentOffset(StringPiece key, uint64_t offset, size_t buffer_size, char* destination, size_t& real_bytes_read) {
  LookupSegItem& seg_item = tmp_lookupseg_items_[string(key)];
  const size_t desired_bytes = std::min(buffer_size, seg_item.total_size);
  if (desired_bytes == 0) {
    real_bytes_read = 0;
...
Status DBReader::ReadCurrent(Tensor* val) {
  CHECK(val != nullptr);
  BundleEntryProto entry;
...
Status DBReader::LookupTensorSlices(StringPiece key,
...
Status DBReader::LookupSlice(StringPiece full_tensor_key,
...
Status DBReader::LookupTensorSliceProtos(
...
Status DBReader::GetSliceValue(StringPiece full_tensor_key,
...
bool DBReader::Contains(StringPiece key) {
  Seek(key);
  return Valid() && (this->key() == key);
}

Status DBReader::LookupDtypeAndShape(StringPiece key, DataType* dtype,
                                         TensorShape* shape) {
  BundleEntryProto entry;
  TF_RETURN_IF_ERROR(GetBundleEntryProto(key, &entry));
  *dtype = entry.dtype();
...
Status DBReader::LookupTensorShape(StringPiece key, TensorShape* shape) {
  DataType ignored;
  return LookupDtypeAndShape(key, &ignored, shape);
}

string DBReader::DebugString() {
...
SegmentDBWriter::SegmentDBWriter(
    DBWriter* writer, const string& name,
...
Status SegmentDBWriter::Begin() {
  return writer_->AddTensorHeader(name_, type_, shape_);
}

Status SegmentDBWriter::WriteData(const void* data, int64 size) {
  while (size > 0) {
...
Status SegmentDBWriter::End() {
  if (write_counter_ * buffer_size_ + buffer_ptr_ !=
      shape_.num_elements() * DataTypeSize(type_)) {
    return errors::Internal("SegmentDBWriter write size error");
  }
  if (write_counter_ == 0) {
...
Status SegmentDBReader::Begin() {
  ...
  if (offset_ + size_ > shape_.dim_size(0)) {
    return errors::InvalidArgument("SegmentDBReader offset error");
  }
...
const TensorShape& SegmentDBReader::shape() {
  return shape_;
}

DataType SegmentDBReader::type() {
  return type_;
}

Status SegmentDBReader::Read(void* data, int64 size) {
  if (size > remain_size_) {
    return errors::InvalidArgument("SegmentDBReader Read Exhuasted");
  }
  size_t read_size;
...
Status SegmentDBReader::Skip(int64 size) {
  if (size > remain_size_) {
    return errors::InvalidArgument("SegmentDBReader Read Exhuasted");
  }
